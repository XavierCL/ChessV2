...
{
public:
	NodeGameSetND(const GameSet& gameSet)
		: _parent(nullptr),
	...
	NodeGameSetND(NodeGameSetND const* parent, const Board& currentBoard, const unsigned char& remainingNonCapturingCount)
		: _parent(parent),
		_rootGameSet(),
	...
	const NodeGameSetND playMove(const Move &move) const
	{
		const Board newBoard(move.play(_currentBoard));

		return NodeGameSetND(
			this,
			newBoard,
