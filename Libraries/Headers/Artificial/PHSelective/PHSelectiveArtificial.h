...
class PHSelectiveArtificial : public Artificial
{
public:

	PHSelectiveArtificial(const unsigned long long &msSelfTime, const size_t &maxNodeCount, std::minstd_rand0 &engine, FixedUnorderedMap<Board, std::shared_ptr<PHSelectiveGameNode>, BoardHash> &nodeRepository, FixedUnorderedMap<Board, std::shared_ptr<std::vector<Move const *>>, BoardHash> &legalCache)
